 ...
 * Class: GuiTools
 * Description: Classe pour cr辿er un menu.
 */
...
function GuiTools(domId, view, w) {
    if (view) {
        ...
        this.colorGui = this.gui.addFolder('レイヤー');
        this.elevationGui = this.gui.addFolder('標高');
        ...
        view.addEventListener('layers-order-changed', (function refreshColorGui() {
            ...
            var geometryLayers = view.getLayers(function filter(l) { return l.isGeometryLayer; });
            for (i = 0; i < geometryLayers.length; i++) {
                this.removeLayersGUI(geometryLayers[i].id);
            }

            this.addImageryLayersGUI(geometryLayers);
        }).bind(this));
    }
}
...
GuiTools.prototype.addImageryLayerGUI = function addImageryLayerGUI(layer) {
    ...
    folder.add({ visible: layer.visible }, 'visible').onChange((function updateVisibility(value) {
        layer.visible = value;
        window.updateLegend(layer.id, layer.visible);
