...
for (const input of base_map_inputs) {
  input.onclick = (layer) => {
    console.log("change base map", layer.target.id );
...
async function make_layers() {
  ...
  addPopLayer(wgapp.map, 'pop_point_cloud', '../../storage/data/download/demographics/sample.csv');
  fflag = 0;
  if (layers_info == null){
  ...
  ***********************************************************/
  $(function () {
    ...
    $('.data_select_box .ctrl_enabled').dblclick(function () {
      console.log('表示から dblclick', this.id);
     ...
     $('.data_select_box .ctrl_enabled').mouseup(function () {
      ...
      try{
        if(rangeSlider != undefined){
          ...
          // if(sourcr_type_list[id] == "vector"){
          //   for (l of layer_jsons[id]["layer"]) {
          //     if (wgapp.map.getLayer(l).type == "fill") {
          //       wgapp.map.setPaintProperty(l, "fill-opacity", rangeSlider.value/10 );
          //     } else if (wgapp.map.getLayer(l).type == "background") {
          //       wgapp.map.setPaintProperty(l, "background-opacity", rangeSlider.value/10 );
          //     } else if (wgapp.map.getLayer(l).type == "line") {
          //       wgapp.map.setPaintProperty(l, "line-opacity", rangeSlider.value/10 );
          //     } else if (wgapp.map.getLayer(l).type == "symbol") {
          //       wgapp.map.setPaintProperty(l, "icon-opacity", rangeSlider.value/10 );
          //       wgapp.map.setPaintProperty(l, "text-opacity", rangeSlider.value/10 );
          //     } else if (wgapp.map.getLayer(l).type == "circle") {
          //       wgapp.map.setPaintProperty(l, "circle-opacity", rangeSlider.value/10 );
          //     } else if (wgapp.map.getLayer(l).type == "fill-extrusion") {
          //       wgapp.map.setPaintProperty(l, "fill-extrusion-opacity", rangeSlider.value/10 );
          //     }
	        //   }
          // }else if(sourcr_type_list[id] == "raster"){
          //   for (l of layer_jsons[id]["layer"]) {
          //     wgapp.map.setPaintProperty(l, "raster-opacity", rangeSlider.value/10 );
          //   }
          // }
        }
      } catch(err) {
...
function setBargraphLayerSource(layerId,sourceLayer) {
  let data = {
...
function addPopLayer(map, layerId, filename){
    setPopLayerSource()
    d3.csv("../../storage/data/download/demographics/sample.csv").then(function(data) {
      wgapp.map.addSource('bargraph_source', {
        "type": "geojson",
        "data": {
          type: 'FeatureCollection',
          features: []
      ...
      wgapp.map.addLayer({
        ...
        'paint': {
          ...
          'fill-extrusion-height': ['get', 'households'],
          'fill-extrusion-base': ['get', 'base'],
      ...
      wgapp.map.addLayer({
        ...
        'paint': {
          ...
          'fill-extrusion-height': ['get', 'housing'],
          'fill-extrusion-base': ['get', 'base'],
          'fill-extrusion-translate': [-10,-10],
          'fill-extrusion-opacity': 0.6
        }
      ...
      wgapp.bargraphUpdate = function(bargraph_data) {
        console.log("wgapp.bargraphUpdate");
      ...
      wgapp.bargraphUpdate(wgapp.bargraph_data);
      updatePopLayer();
...
function setLayerVisible(layerIds){
  ...
  console.log("setLayerVisible", url_select_layerids)
}
